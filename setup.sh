...
#
# 6. ZDOTDIR ya NO se fija escribiendo en /etc/zsh/zshenv (ni /etc/zshenv).
#    La ruta del "global zshenv" está COMPILADA en el binario de zsh y varía
#    según de dónde venga ese zsh: el zsh de sistema de Apple usa /etc/zshenv,
#    zsh de Homebrew o de paquetes tipo Debian pueden usar /etc/zsh/zshenv.
#    Adivinar cuál te toca es frágil. En su lugar, usamos el mecanismo
#    documentado y portable de zsh: ~/.zshenv siempre se lee primero (ya que
#    ZDOTDIR no está definido todavía y por defecto vale $HOME), y ESE archivo
#    es el que fija ZDOTDIR para todo lo que se lee después (.zprofile,
#    .zshrc, .zlogin). No requiere sudo ni depende del sistema operativo.
# ---------------------------------------------------------------------------

set -euo pipefail
trap 'echo "[setup][ERROR] El script falló en la línea $LINENO (comando: $BASH_COMMAND)" >&2' ERR
...
if ! command -v git >/dev/null 2>&1; then
  echo "[setup][ERROR] git no está disponible. Instala Xcode Command Line Tools: xcode-select --install" >&2
  exit 1
fi
...
#    Una sola consulta de lo instalado + una sola instalación de lo que falta.
#    (Antes eran ~28 invocaciones de brew; cada invocación lanza decenas de
#    procesos internos, y en un sistema cerca de su límite de procesos eso
#    multiplica el riesgo de "fork: Resource temporarily unavailable".)
# ---------------------------------------------------------------------------
if command -v brew >/dev/null 2>&1; then
  ...
  if [ "${#MISSING[@]}" -gt 0 ]; then
    log "Instalando con Homebrew: ${MISSING[*]}"
    brew install "${MISSING[@]}" || warn "brew install terminó con errores; revisa la salida de arriba. Continúo con el resto del setup."
  else
    log "Todos los paquetes de Homebrew ya están instalados."
  fi
else
...
# Este archivo se lee como ~/.zshenv (symlink) ANTES de que ZDOTDIR exista —
# es lo que redirige el resto del arranque de zsh (.zprofile, .zshrc, .zlogin)
# hacia esta carpeta. No lo muevas ni lo renombres.
...
# (setup.sh lo clona; aquí solo se usa si existe — nada de clonar al arrancar
# la shell, que es lento y falla en silencio sin red.)
...
# ---------------------------------------------------------------------------
# 7. Symlinks del repo hacia ~/.config
# ---------------------------------------------------------------------------
link_config "$REPO_ROOT/.config/zsh"  "$HOME/.config/zsh"

# Symlink de archivo suelto (no de directorio): esto es lo que hace que
# ZDOTDIR se aplique de verdad. zsh lee ~/.zshenv por defecto ANTES de saber
# que existe otro ZDOTDIR — este archivo es el que se lo dice.
...
# ---------------------------------------------------------------------------
# 8. Plugins de zsh — clonados AQUÍ, no en el arranque de la shell.
#    plugins.zsh mantiene el clone como fallback, pero la ruta normal es que
#    al abrir la shell ya esté todo en disco. Errores visibles, no silenciados.
# ---------------------------------------------------------------------------
...
for plugin in "${ZSH_PLUGINS[@]}"; do
  ...
  if [ ! -d "$ZSH_PLUGINS_DIR/$name" ]; then
    log "Clonando plugin de zsh: $name"
    git clone --depth 1 "https://github.com/$plugin.git" "$ZSH_PLUGINS_DIR/$name" \
      || warn "No se pudo clonar $plugin — la shell lo reintentará al arrancar."
  fi
done
