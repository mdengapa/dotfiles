...
- Instala vía Homebrew: `zsh`, `tmux`, `neovim`, `starship`, `fzf`, `zoxide`,
  ...
  (en una sola invocación de brew, solo lo que falte).
- Fija `ZDOTDIR=~/.config/zsh` enlazando `~/.zshenv` al `.zshenv` del repo —
  sin tocar `/etc` y sin `sudo`. Es el mecanismo portable de zsh: `~/.zshenv`
  se lee siempre primero, y es ese archivo el que redirige el resto del
  arranque (`.zprofile`, `.zshrc`) hacia `~/.config/zsh`.
- Clona los plugins de zsh (fzf-tab, autosuggestions, syntax-highlighting,
  history-substring-search, vi-mode, zsh-completions) en
  `~/.local/share/zsh/plugins` — el arranque de la shell nunca necesita red.
